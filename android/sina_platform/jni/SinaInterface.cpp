...
extern "C"
{
	...
	*/
	void SinaLogin()
    {
		...
		if (JniHelper::getStaticMethodInfo(t
			, "com/izhangxin/platform/sina/SinaHandler"
			, "SinaPlatformLogin"
			, "()V"))
		{
		...
		{
			CCAssert(false, "com.izhangxin.platform.sina.SinaHandler not valid!");
		}
    }
	...
	*/
	...
	void Java_com_izhangxin_platform_sina_SinaHandler_onSinaLoginFail(JNIEnv*  env, jobject obj)
    {
	...
	void Java_com_izhangxin_platform_sina_SinaHandler_onSinaLoginCancel(JNIEnv*  env, jobject obj)
    {
    	OnPushEventToLoadingLayerCallback* callback = new OnPushEventToLoadingLayerCallback(UPDATE_EVENT_ND_LOGIN_FAIL);
    	callback->autorelease();
    	CCScheduler::sharedScheduler()->scheduleSelector(schedule_selector(OnPushEventToLoadingLayerCallback::callback), callback, 0, false);
    }
	void Java_com_izhangxin_platform_sina_SinaHandler_onSinaLoginSuccess(JNIEnv*  env, jobject obj,jstring uin, jstring sid)
    {
		string strUin = JniHelper::jstring2string(uin); 
    	string strSessionId = JniHelper::jstring2string(sid); 
		CCLOG("Java_com_izhangxin_platform_sina_SinaHandler_onSinaLoginSuccess:%s,%s", strUin.c_str(), strSessionId.c_str());
