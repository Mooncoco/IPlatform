...
public:
	void callback(ccTime time)
	{
		...
		else if(m_nCode == PAY_SMS_OK)
			nResultStatus = ChargeResultLayer::kResultSMSOk;
...
extern "C"
{
	void IPlatformPayInit()
	{
		...
		if (JniHelper::getStaticMethodInfo(t
			, "com/izhangxin/dx/android/handler/IPlatformHandler"
			, "IPlatformPayInit"
			, "()V"))
		{
		...
		{
			CCAssert(false, "com.izhangxin.dx.android.handler.IPlatformHandler not valid!");
		}
	}
	...
	void IPlatformPayQuick(const char* order, Chest chest,int gameId) 
	{
		...
		if (JniHelper::getStaticMethodInfo(t
			, "com/izhangxin/dx/android/handler/IPlatformHandler"
			, "IPlatformPayQuick"
			, "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;FFII)V"))
		{
			jstring pOrder = t.env->NewStringUTF(order);
			jstring pProductName = t.env->NewStringUTF(chest.goods_name_.c_str());
			jstring pProductDesc = t.env->NewStringUTF(chest.desc_.c_str());
			...
			int nProductCount = 1;
			int nGameId = gameId;
			t.env->CallStaticIntMethod(t.classID, t.methodID, pOrder, pProductName, pProductDesc, pProductSerino, fProductPrice, fProductOrignalPrice,nProductCount,nGameId );
			t.env->DeleteLocalRef(t.classID);
			t.env->DeleteLocalRef(pOrder);
			t.env->DeleteLocalRef(pProductName);
			t.env->DeleteLocalRef(pProductDesc);
	...
	void IPlatformInitHeadFace() 
	{
		JniMethodInfo t;
		if (JniHelper::getStaticMethodInfo(t
			, "com/izhangxin/bdengine/AndroidGameInterface"
			, "initPhotoWithPath"
			, "(Ljava/lang/String;)V"))
		{
		...
		{
			CCAssert(false, "com.izhangxin.bdengine.AndroidGameInterface not valid!");
		}
	}

	void Java_com_izhangxin_dx_android_handler_IPlatformHandler_onPayResult(JNIEnv*  env, jobject obj, jint code)
	{
